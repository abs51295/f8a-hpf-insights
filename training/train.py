...
def make_user_item_df(manifest_dict, package_dict, user_input_stacks):
    ...
    for stack in user_input_stacks:
        set_input_stacks.add(frozenset([package_dict.get(x) for x in stack]))
    logger.info("Set of user input stacks is: {}".format(set_input_stacks))
    id_package_dict = {v: k for k, v in package_dict.items()}
    for k, v in manifest_dict.items():
        user_id = int(k)
        is_user_input_stack = frozenset(v) in set_input_stacks
        # logger.info("Value of is_user_input_stack is: {}".format(is_user_input_stack))
        for package in v:
